            ...
            for encode_face, face_Loc in zip(encoding_cur_frame, face_cur_frame):
                ...
                if matches[match_index]:
                    ...
                    if name == self.user_index:
                        ...
                        cv2.putText(image, 'Nobody', (x1 + 6, y2 - 6), cv2.FONT_HERSHEY_COMPLEX, 1,
                                    (255, 255, 255), 2)
        else:
            self.now_time = 0
...
class Camera:
    ...
    def GUI(self, window, mode):
        ...
        self.image_tk = ImageTk.PhotoImage(image_F)
        self.canvas = tk.Canvas(window, width=cam_width, height=cam_height, bg='black')
        self.canvas.place(x=5, y=5)
        self.canvas_img = self.canvas.create_image(cam_width / 2 + 2, cam_height / 2 + 2,
                                                   image=self.image_tk)
        self.canvas_run = self.canvas.after(1, self.canvas_after)

    def canvas_after(self):
        ret, image_org = self.cam.read()

        image = self.image_function(image_org)
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
        image_F = Image.fromarray(image)
        self.image_tk = ImageTk.PhotoImage(image_F)
        self.canvas.itemconfig(self.canvas_img, image=self.image_tk)
        self.canvas_run = self.canvas.after(1, self.canvas_after)

    def image_function(self, image_org):
        if self.mode == 'Normal':
            ...
            if self.Tracking_Enable:
                image = self.Tracking_System.digital_finger(image_org)
            else:
                image = self.Tracking_System.nor_tracking(image_org)
            return image
        elif self.mode == 'Recognition':
            FRC.Recognition(image_org)
...
class Main_GUI:
    ...
    def Activate_Vision_System(self):
        Vision.GUI(self.main_window, 'Tracking')
