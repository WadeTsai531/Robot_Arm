...
class Vision_System:
    def __init__(self, index_camera):
        # ----- Variable -----
        self.image_F = None
        self.image_tk = None
        self.canvas = None
        ...
        # ----- Camera Setup -----
        self.cam = cv2.VideoCapture(index_camera)
    ...
    def Capture(self):
        name = Logout_Create.username
        image = self.save_image
        cv2.imwrite('test_3/' + name + '.jpg', image)
        ...
        if self.username_now == '' or self.password_now == '':
            ...
            if data is None:
                ...
                if data['UserName'] == self.username_now and data['Password'] == self.password_now:
                    self.user_right = data['Right']
                    print('Correct')
                    print('Login System: Right:', self.user_right)
...
class Logout_Create_System:
    ...
    def Register(self):
        if MF.get('Face Detection', self.username) is None:
            ...
            user_list.append({'Name': self.username, 'Right': 'User'})
            data = {'User': user_list}
    ...
    def digital_finger(self, image):
        ...
        if len(my_list) != 0:
            ...
            def Finger_up_down(RL_index):
                ...
                for index, function_name in enumerate(finger_list[:5]):
                    if finger_distance[index] > 13:
                        ...
                        function_name.Vision_function(120)  # Hardware Finger Down
                    elif finger_distance[index] < -13:
                        finger_detect.append(False)
                        function_name.Vision_function(5)  # Hardware Finger Up
            ...
            def Arm_up_down(index):
                ...
                if 138 <= abs(y) <= 380:
                    finger_list[5].Vision_function(int((380 - abs(y)) * (13 / 24)))
...
class Finger_System:
    def __init__(self, text, port, max_value=130):
        # -------- Variable define --------
    ...
    def Spinbox_function(self):
        ...
        if self.label_text == 'Arm':
            Serial.Transmit_value(self.transmit_port, int(spin_value) + 22)
        else:
            Serial.Transmit_value(self.transmit_port, 130 - int(spin_value) + 22)
    ...
    def Scale_function(self, v):
        ...
        if self.label_text == 'Arm':
            Serial.Transmit_value(self.transmit_port, scale_value + 22)
        else:
            Serial.Transmit_value(self.transmit_port, 130 - scale_value + 22)

    def Vision_function(self, input_value):
        # print('Vision:', input_value)
        self.sub_scale.set(120 - input_value)
        self.deg.set(120 - input_value)
        self.Change_Color(120 - input_value)

    def Change_Color(self, input_value):
        color_number = str(hex(abs((120 - int(input_value)) * 25 // 13)))
...
class Main_GUI:
    ...
    def GUI(self):
        ...
        right_label = tk.Label(self.main_window, text='Right: ' + Login.user_right,
                               font=('Times', 20, 'bold'))
